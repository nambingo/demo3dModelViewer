...
import com.example.framgiaphamducnam.demo3dmodelview.collision.Octree;
...
public class Object3DData {
	...
	// derived data
	private com.example.framgiaphamducnam.demo3dmodelview.entities.BoundingBox boundingBox;
	...
	public Object3DData andScaleHead(float maxSize, float yc){
		float leftPt = Float.MAX_VALUE, rightPt = Float.MIN_VALUE; // on x-axis
		float topPt = Float.MIN_VALUE, bottomPt = Float.MAX_VALUE; // on y-axis
		float farPt = Float.MAX_VALUE, nearPt = Float.MIN_VALUE; // on z-axis
		...
		float zc = (nearPt + farPt) / 2.0f;
		//yc = -8f;
		Log.e("Object3DData", "andScale:  -----> xc: "+xc + " - yc: "+yc + " - zc: "+zc);

		// this.setOriginalPosition(new float[]{-xc,-yc,-zc});

		// calculate largest dimension
		float height = topPt - bottomPt;
		Log.e("Object3DData", "andScale:  -----> height: "+height);
		float depth = nearPt - farPt;
		float largest = rightPt - leftPt;
		if (height > largest)
			largest = height;
		...
		if (largest != 0.0f)
			scaleFactor = (maxSize / largest);
		Log.e("Object3DData",
				"Centering & scaling --> '" + getId() + "' to (" + xc + "," + yc + "," + zc + ") "
						+ "scale: '" + scaleFactor + "'");
	...
	public Object3DData andScaleLeft(float maxSize, float xc, float yc, float zc){
		float leftPt = Float.MAX_VALUE, rightPt = Float.MIN_VALUE; // on x-axis
		float topPt = Float.MIN_VALUE, bottomPt = Float.MAX_VALUE; // on y-axis
		float farPt = Float.MAX_VALUE, nearPt = Float.MIN_VALUE; // on z-axis
	...
	public com.example.framgiaphamducnam.demo3dmodelview.entities.BoundingBox getBoundingBox() {
		FloatBuffer vertexBuffer = getVertexBuffer();
		if (vertexBuffer == null){
			vertexBuffer = getVertexArrayBuffer();
		}
		if (boundingBox == null) {
			boundingBox = com.example.framgiaphamducnam.demo3dmodelview.entities.BoundingBox.create(getId()+"_BoundingBox", vertexBuffer,
					getModelMatrix());
		}
		return boundingBox;
	}
